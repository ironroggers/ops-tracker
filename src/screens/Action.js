...
function RightActionsPanel({
  ...
  React.useEffect(() => {
    ...
    const allIds = causeActions.flatMap((cause) =>
      cause.actions.map((a) => a.id)
    );
    const resolved = allIds.every((id) =>
      ["approved", "dismissed"].includes(actionStatuses[id])
    );
        ...
        <div className="report-causes-grid">
          ...
          <article className="report-cause">
            ...
            <div className="report-mini">
              ...
              <div className="report-mini-stat">
                <div className="report-mini-stat-val">+ 22%</div>
                <div className="report-mini-stat-sub">in last 3 months</div>
              </div>
            </div>
          </article>
...
function ActionExecutionPanel({
  ...
  const startAIGeneration = (actionId, actionText) => {
    ...
    const phase1 = setTimeout(() => {
      setExecutionInfo((prev) => ({
        ...prev,
        [actionId]: "Drafting outline…",
      }));
    }, randomMs(900, 1400));
    const phase2 = setTimeout(() => {
      setExecutionInfo((prev) => ({
        ...prev,
        [actionId]: "Writing first draft…",
      }));
    }, randomMs(1800, 2400));
  ...
  const startExecutionTimeline = (actionId, actionType) => {
    ...
    switch (actionType) {
      case "meeting":
        ...
        stepDuration = randomMs(
          TIMELINE_STEP_MS + 800,
          TIMELINE_STEP_MS + 1800
        );
        break;
      case "wo-permit":
        {
      ...
      <div className="action-content">
        {phase === "input" && (
          <div className="action-card" role="region" aria-label="Assistant">
            <h1 className="action-title">Let's dive deep into the problem</h1>
